...
@section('content_header')
    <h1 class="mb-3">
        ...
        {{ isset($enrollment) ? 'Edit Enrollment' : 'Add Enrollment' }}
    </h1>
@stop
...
@section('content')
    <div class="card shadow">
        <div class="card-body">
            <form action="{{ isset($enrollment) ? route('enrollments.update', $enrollment->id) : route('enrollments.store') }}" 
                  method="POST">
                @csrf
                @if(isset($enrollment))
                    @method('PUT')
                        ...
                        <select name="academic_session_id" class="form-control">
                            ...
                            @foreach($sessions as $session)
                                <option value="{{ $session->id }}" 
                                    {{ (old('academic_session_id') ?? $enrollment->academic_session_id ?? '') == $session->id ? 'selected' : '' }}>
                                    {{ $session->name }}
                                </option>
                            @endforeach
                        </select>
