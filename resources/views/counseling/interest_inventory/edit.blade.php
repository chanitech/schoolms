...
@section('title', 'Edit Interest Inventory')

@section('content_header')
    <h1><i class="fas fa-edit"></i> Edit Interest Inventory</h1>
@stop
...
<div class="card">
    ...
    <form action="{{ route('interest-inventories.update', $interestInventory->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="student_id" class="form-control" required>
                        @foreach($students as $student)
                            <option value="{{ $student->id }}" {{ $interestInventory->student_id == $student->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    <label>Date</label>
                    <input type="date" name="date" class="form-control" value="{{ $interestInventory->date?->format('Y-m-d') }}">
                </div>
            </div>
            ...
            @foreach(range(1,17) as $i)
                <div class="form-group mb-3">
                    ...
                    <textarea name="q{{ $i }}" id="q{{ $i }}" class="form-control" rows="2">{{ old("q{$i}", $interestInventory->{"q{$i}"}) }}</textarea>
                </div>
            @endforeach
        </div>
