...
<div class="container-fluid px-3 py-2">
    ...
    <div class="filter-bar">
        <form action="{{ route('results.show', $student->id) }}" method="GET" id="filterForm">
...
function loadExamsForSession() {
    ...
    const selectedExam = "{{ $selected_exam_id }}";   // preserve previous selection

    if (!sessionId) {
        // If “All Sessions” is chosen, show all exams that are already in the page
        Array.from(examSel.options).forEach(o => {
            if (o.value) o.style.display = '';
        });
        return;
    }
    ...
    fetch("{{ route('marks.exams.by.session') }}?session_id=" + sessionId)
        ...
        .then(exams => {
            ...
            if (exams.length > 0) {
                exams.forEach(exam => {
                    const sel = (exam.id == selectedExam) ? ' selected' : '';
...
document.getElementById('sessionSel').addEventListener('change', loadExamsForSession);
